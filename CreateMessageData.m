function message_data = CreateMessageData( sv_selected )
    ...
    for selected_sv_count = 1:4
        if isempty( dir( '*.alm' ) ) ~= 1
            % Select s single SV from the list of selected SVs
            selected_sv = sv_selected( selected_sv_count );
            % Index the selected SV.
            index_selected_sv = find( full_almanac_data == selected_sv );
        else
            % Fetch Yuma almanac data
        ...
        message_data =  [ message_data; temp_message_data];
    end
    ...
    % end
    %
    fprintf('Message data for satelitte %d, %d, %d, and %d has been generated.\n', sv_selected(1), sv_selected(2), sv_selected(3), sv_selected(4) );
